...
an optional cache to squeeze even more performance out.
...
Arguments supplied to dispatcher (except first) are available to executed vector
...
matcher to a subroutine to call on a successful match of the input.
