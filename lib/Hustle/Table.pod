...
Call dispatcher to return the matching entries and any regex captures

  my @results=$dispatch->("thing to match");	
...
From v0.7.0 It returns a list containing pairs of matching entries and an
anonymous array of any captures from regex matching if applicable. 
