...
Array entries must contain four elements, in the order of:
	...
	add([$matcher,$sub,$label,$count]);

Hashes ref format only need to specify the matcher and sub pairs

	add({matcher=>$matcher, sub=>$sub, label=>$label, count=>$count});

Single flattened format takes a list directly

	add(matcher=>$matcher, sub => $sub);

Or add using formats together
...
It C<$sub> must also be a CODE reference. 
