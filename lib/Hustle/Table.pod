...
=head1 SYNOPSIS
  ...
  # context:		optional. User context passed as first argument to sub
  # type		optional. default is regex. options include: start/end/exact/numeric
  ...
  $table->add(
	...
	[qr/another/, sub{}, "my label", 10,{}, undef],			
	
	#undef matcher is default match all. Must be array ref format
	[undef,sub {},"default",undef,undef,undef]				
  );
...
Single table matches with any combination of regex, exact string, start string,
end string or numeric comparisons
...
Conceptually the list is looped over, testing the input against the matchers
sequentially to the input until a match is found. In practice it isn't a loop,
but generated code reference with potentially optimised ordering of the
entries.
...
B<Version 0.5.1 and later>
Added support for multimatching.

B<Version 0.5.0 and later>
A single dispatch 'type' is now used in either cached or uncached variants.
The 'offline' type has been removed
...
The input string (the first argument to the dispatcher) is removed and replaced with
...
If no C<type> is specified or is C<undef>, the C<matcher> is always treated as a regex
...
removed from the cache, if a cache is used 


In multimatch mode, the C<sub> can actually be any scalar.
...
is used as a priority. Higher numeric values are a higher priority. If the
C<reorder> option is specified when preparing a dispatcher, the list is sorted
in descending order of priority, meaning the highest priority is the first
element in the list.
...
to reorder the list. If the C<reset> option is specified when
prepare_dispatcher, the C<count> field is reset to 0
...
is  the first argument to the called subroutine
	...
	ie 
		sub {
			my ($matcher, $optional, $args)=@_;
			$matcher->[4]; #This is the context
...
are:
	...
	"start"	=>	matches the begining of input string
	"end"	=>	matches the end of input string
	"exact"	=>	string equality
	"numeric" =>	numeric equality

If C<matcher> is indeed a regex, C<type> is ignored. 
...
Anonymous array entries must contain six elements, in the order of:
	...
	$table->add([$matcher, $sub, $label, $count, $ctx, $type]);

Anonymous hashes format only need to specify the matcher and sub pairs

	$table->add({matcher=>$matcher, sub=>$sub, label=>$label, count=>$count, ctx=>$ctx, type=>$type});
...
This makes the dispatcher return a list of matched entries.  Processing of the
list is then upto to the user. Caching is unsupported in this mode
...
defaults:
	...
	my $dispatcher=$table->prepare_dispatcher(cache=>undef, reset=>undef, reorder=>1, multimatch=>undef);

=head1 USING A DISPATCHER

=head2 Normal/Single Match
...
In Multimatch mode, the matches are returns as a list. You can access the sub field as item 1 in the matcher. The example below calls the C<sub> in each matcher in the same format it would have been called in single match mode:

	 my @matches=$dispatcher->("input"); 
	 for(@matches){
	 	$_->[1]($_, "args", "for", "vector");
	}

Note that in multimatch mode, the C<sub> can be any scalar vector.
...
the distribution of input frequency does play an important role in determining
the best way perform the search.
